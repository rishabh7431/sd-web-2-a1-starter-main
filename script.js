...
const brokenCharacters = [
  { id: 1, name: "Luke Skywalker", age: 23 },
  { id: 2, name: "Darth Vader", age: 45 },
  { id: 3, name: null, age: 23 },
  { id: 4, name: "Obi-Wan Kenobi", age: 57 },
  { id: 5, name: "Yoda", age: 900 },
  { id: 6, name: null , age: 32 },
  { id: 7, name: "Chewbacca", age: 234 },
  { id: 8, name: "R2-D2", age: 33 },
  { id: 9, name: "C-3PO", age: 112 },
  { id: 10, name: null, age: 27 },
];
...
function filterArray() {
  ...
  for (let i = 0; i < characters.length; i++) {
    if (characters[i].age < 40) {
      youngCharacters.push(characters[i]);
    }
  }
  for (let i = 0; i < youngCharacters.length; i++) {
    console.log(youngCharacters[i].name);
    const li = document.createElement("li");
    li.textContent = youngCharacters[i].name;
    list.appendChild(li);
  }
}
...
function renderCharacterNames(characterArray) {
  ...
  for (let i = 0; i < characterArray.length; i++) {
    if (!characterArray[i].name) {
      showError(`Error: Character at index ${i} is missing a name property.`);
      continue;
    }
    const li = document.createElement("li");
    li.textContent = characterArray[i].name;
    list.appendChild(li);
  }
}
...
function renderCharactersByAge(characterArray, ageThreshold) {
  const list = document.getElementById("age-filter-list");
  list.innerHTML = "";
  for (let i = 0; i < characterArray.length; i++) {
    if (characterArray[i].age < ageThreshold) {
      if (!characterArray[i].name) {
        showError(`Error: Character under age ${ageThreshold} at index ${i} is missing a name property.`);
        continue;
      }
      const li = document.createElement("li");
      li.textContent = characterArray[i].name;
